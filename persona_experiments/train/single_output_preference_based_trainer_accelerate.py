...
from persona_experiments.train.accelerate_trainer import AccelerateTrainer


class SingleOutputPreferenceBasedTrainerAccelerate(AccelerateTrainer):
    """
    Trainer for preference-based objective. Currently supports DPO, IPO, and cross entropy using the preferred outputs.
    """

    def __init__(self, model, tokenizer, optimizer, accelerator, kl_coeff: float = 0.1, objective: str = "dpo", train_evaluator=VoidEvaluator(),
                 val_evaluator=VoidEvaluator(), callback=None, track_logits_for_tokens: List[str] = None,
                 gradient_accumulation: int = -1):
        super().__init__(model, optimizer, train_evaluator, val_evaluator, callback)
    ...
    def batch_update(self, batch_num, batch, total_num_batches):
        ...
        preferred_output_ids = batch["preferred_output_ids"]
        dispreferred_output_ids = batch["dispreferred_output_ids"]
        ref_preferred_logprobs = batch["ref_preferred_logprobs"]
        ref_dispreferred_logprobs = batch["ref_dispreferred_logprobs"]
